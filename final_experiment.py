...
def solve_min_node_set_ip(G, terminal_indices, D, time_limit=300, verbose=True):
    """
    Find smallest S ⊆ V(G) containing terminal_indices such that
    G.subgraph(S) supports at least D[(o,d)] units of flow between each (o,d) pair.

    Args:
        G:                networkx Graph
        terminal_indices: nodes that must belong to S (e.g. [la, sf, phx, dal])
        D:                dict {(origin, dest): demand} — one commodity per pair
    ...
    Formulation: multi-commodity flow with binary node-selection variables.
      ...
      f[(o,d),u,v] ∈ [0, D[o,d]] — flow on arc u→v for commodity (o,d)
      Conservation: net outflow = D[o,d] at o, −D[o,d] at d, 0 elsewhere
      Coupling: f[(o,d),u,v] ≤ D[o,d]·x[u]  and  f[(o,d),u,v] ≤ D[o,d]·x[v]
    ...
    f = {}
    for (o, d), demand in D.items():
        for u in nodes:
            for v in nbrs[u]:
                f[(o, d, u, v)] = m.addVar(lb=0.0, ub=demand, name=f"f_{o}_{d}_{u}_{v}")
    m.update()
    ...
    # --- fix terminals ---
    for t in terminal_indices:
        m.addConstr(x[t] == 1, name=f"fix_{t}")

    # --- flow conservation + arc-node coupling, one block per commodity ---
    for (o, d), demand in D.items():
        for i in nodes:
            out_f = gp.quicksum(f[(o, d, i, j)] for j in nbrs[i])
            in_f  = gp.quicksum(f[(o, d, j, i)] for j in nbrs[i])
            rhs   = demand if i == o else (-demand if i == d else 0)
            m.addConstr(out_f - in_f == rhs, name=f"flow_{o}_{d}_{i}")

        for u in nodes:
            for v in nbrs[u]:
                m.addConstr(f[(o, d, u, v)] <= demand * x[u], name=f"tail_{o}_{d}_{u}_{v}")
                m.addConstr(f[(o, d, u, v)] <= demand * x[v], name=f"head_{o}_{d}_{u}_{v}")
...
other_nodes_df = pd.read_csv("data/D_parallel_out_flat.csv")
...
# bat threshold: bat equivalent of EDGE_THRESHOLD miles
# dist in the dataframe is in meters; derive bat-per-meter from positive rows
_pos = other_nodes_df[(other_nodes_df['bat'] > 0) & (other_nodes_df['dist'] > 0)]
_bat_per_meter = (_pos['bat'] / _pos['dist']).median()
BAT_THRESHOLD = _bat_per_meter * EDGE_THRESHOLD * 1609.34  # EDGE_THRESHOLD miles → meters
...
for _, row in other_nodes_df.iterrows():
    ...
    if not N.has_edge(i, j) or N[i][j]['bat'] > bat:
        N.add_edge(i, j, weight=row['dist'] / 1609.34, bat=bat)
...
# ============================================================
# Solve minimum-cardinality Steiner node set IP
# ============================================================
terminals = [la_idx, sf_idx, phx_idx, dal_idx]
print(f"\nSolving min-node-set IP over {len(N.nodes)} nodes, {len(N.edges)} edges ...")
S, obj, status = solve_min_node_set_ip(N, terminals, D=parameters['D'], time_limit=300, verbose=True)
print(f"\nIP status : {status}")
print(f"Min |S|   : {obj}")
print(f"Nodes in S: {S}")
S_coords = [all_selected_nodes[i] for i in S]
print(f"Coords    : {S_coords}")
