...
class TestC1fa89f5Instance(unittest.TestCase):
    ...
    def test_ddd_run_default(self):
        soln = self.test_run_ddd_heuristic()
        x_load = soln['x_load']
        T = 65  # must match build_instance_from_coords

        # Extract the source/sink used by the heuristic solution so the default
        # instance has consistent demand constraints with the seeded breakpoints.
        ...
        for e, flow in x_load.items():
            if flow > 0:
                ...
                if e[1][2] == T - 1:
                    sink = e[1][0]
        self.assertIsNotNone(source, "Could not determine source from heuristic x_load")
        self.assertIsNotNone(sink,   "Could not determine sink from heuristic x_load")
        ...
        instance.seed(x_load)
        new_sol, val, props = instance.run_DDD()
        self.assertIsNotNone(new_sol)
        self.assertIsNotNone(val)
        print(f"c1fa89f5 (default seeded) — objective={val}, props={props}")
